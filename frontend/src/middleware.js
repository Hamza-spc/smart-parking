...
export default auth((req) => {
    ...
    const isBackendApiRoute = nextUrl.pathname.startsWith('/backend-api')
    const isPublicRoute = publicRoutes.includes(nextUrl.pathname)
    const isAuthRoute = authRoutes.includes(nextUrl.pathname)

    /** Skip auth middleware for api routes and backend proxy routes */
    if (isApiAuthRoute || isBackendApiRoute) return
